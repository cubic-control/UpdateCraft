...
public class EntityRabbit extends EntityAnimal{
	...
	protected String getJumpingSound() {
		return RefStrings.MODID + ":mob.rabbit.hop";
	}
	@Override
	protected String getLivingSound() {
		return RefStrings.MODID + ":mob.rabbit.idle";
	}
	@Override
	protected String getHurtSound() {
		return RefStrings.MODID + ":mob.rabbit.hurt";
	}
	@Override
	protected String getDeathSound() {
		return RefStrings.MODID + ":mob.rabbit.death";
	}
	@Override
	protected void dropFewItems(boolean hitRencently, int fortune) {
		...
		for(int i = 0; i < j; i++){
			dropItem(MItems.rabbit_hide, 1);
		}
		j = rand.nextInt(2);
		
		for(int i = 0; i < j; i++){
			if(isBurning()){
				dropItem(MItems.cooked_rabbit, 1);
			}else{
				dropItem(MItems.rabbit, 1);
			}
		}
		if(rand.nextInt(100) <= 10 + fortune){
			entityDropItem(new ItemStack(MItems.rabbit_foot), 0.0F);
		}
	}
