...
public class AdvancementProgress implements Comparable<AdvancementProgress>
{
    ...
    public void func_192104_a(PacketBuffer p_192104_1_) throws IOException
    {
        p_192104_1_.writeVarIntToBuffer(this.criteria.size());

        for (Entry<String, CriterionProgress> entry : this.criteria.entrySet())
        {
            p_192104_1_.writeStringToBuffer(entry.getKey());
            ((CriterionProgress)entry.getValue()).write(p_192104_1_);
        }
    }

    public static AdvancementProgress func_192100_b(PacketBuffer p_192100_0_) throws IOException
    {
        ...
        for (int j = 0; j < i; ++j)
        {
            advancementprogress.criteria.put(p_192100_0_.readStringFromBuffer(32767), CriterionProgress.read(p_192100_0_, advancementprogress));
        }
    ...
    public Date func_193128_g()
    {
        ...
        for (CriterionProgress criterionprogress : this.criteria.values())
        {
            if (criterionprogress.isObtained() && (date == null || criterionprogress.func_193140_d().before(date)))
            {
    ...
    public static class Serializer implements JsonDeserializer<AdvancementProgress>, JsonSerializer<AdvancementProgress>
        {
            ...
            public AdvancementProgress deserialize(JsonElement p_deserialize_1_, Type p_deserialize_2_, JsonDeserializationContext p_deserialize_3_) throws JsonParseException
            {
                ...
                JsonObject jsonobject1 = JsonUtils.getJsonObjectFieldOrDefault(jsonobject, "criteria", new JsonObject());
                AdvancementProgress advancementprogress = new AdvancementProgress();

                for (Entry<String, JsonElement> entry : jsonobject1.entrySet())
                {
                    String s = entry.getKey();
                    advancementprogress.criteria.put(s, CriterionProgress.fromDateTime(advancementprogress, JsonUtils.getJsonElementStringValue(entry.getValue(), s)));
                }
