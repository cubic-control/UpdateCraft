...
public class Advancement{
    ...
    private final IChatComponent displayText;
    ...
    public Advancement(ResourceLocation id, @Nullable Advancement parentIn, @Nullable DisplayInfo displayIn, /*AdvancementRewards rewardsIn,*/ Map<String, Criterion> criteriaIn, String[][] requirementsIn)
    {
      ...
      //TODO:
        ...
        if (displayIn == null)
        {
            this.displayText = new ChatComponentText(id.toString());
        }
        else
        {
            this.displayText = new ChatComponentText("[");
            this.displayText.getChatStyle().setColor(displayIn.getFrame().getFormat());
            IChatComponent itextcomponent = displayIn.getTitle().createCopy();
            IChatComponent itextcomponent1 = new ChatComponentText("");
            IChatComponent itextcomponent2 = itextcomponent.createCopy();
            itextcomponent2.getChatStyle().setColor(displayIn.getFrame().getFormat());
            ...
            itextcomponent.getChatStyle().setChatHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, itextcomponent1));
  ...
  //TODO:
    ...
    public IChatComponent getDisplayText()
    {
    ...
    public static class Builder
        {
          ...
          //TODO:
            ...
            public void writeTo(PacketBuffer buf) throws IOException{
                ...
                Criterion.func_192141_a(this.criteria, buf);
                buf.writeVarIntToBuffer(this.requirements.length);

                for(String[] astring : this.requirements){
                    buf.writeVarIntToBuffer(astring.length);
            ...
            public static Advancement.Builder deserialize(JsonObject json, JsonDeserializationContext context){
                ResourceLocation resourcelocation = json.has("parent") ? new ResourceLocation(JsonUtils.getJsonElementStringValue(json, "parent")) : null;
                DisplayInfo displayinfo = json.has("display") ? DisplayInfo.deserialize(JsonUtils.getJsonElementAsJsonObject(json, "display"), context) : null;
                //TODO:
                //AdvancementRewards advancementrewards = /*(AdvancementRewards)JsonUtils.deserializeClass(json, "rewards", AdvancementRewards.EMPTY, context, AdvancementRewards.class)*/null;
                Map<String, Criterion> map = Criterion.func_192144_b(JsonUtils.getJsonElementAsJsonObject(json, "criteria"), context);

                if(map.isEmpty()){
                    throw new JsonSyntaxException("Advancement criteria cannot be empty");
                }else{
                    JsonArray jsonarray = JsonUtils.getJsonObjectJsonArrayFieldOrDefault(json, "requirements", new JsonArray());
                    String[][] astring = new String[jsonarray.size()][];

                    for(int i = 0; i < jsonarray.size(); ++i){
                        JsonArray jsonarray1 = JsonUtils.getJsonObjectJsonArrayField((JsonObject) jsonarray.get(i), "requirements[" + i + "]");
                        astring[i] = new String[jsonarray1.size()];

                        for(int j = 0; j < jsonarray1.size(); ++j){
                            astring[i][j] = JsonUtils.getJsonElementStringValue(jsonarray1.get(j), "requirements[" + i + "][" + j + "]");
                        }
                    }
                    if(astring.length == 0){
            ...
            public static Advancement.Builder readFrom(PacketBuffer buf) throws IOException{
                ...
                Map<String, Criterion> map = Criterion.func_192142_c(buf);
                String[][] astring = new String[buf.readVarIntFromBuffer()][];

                for (int i = 0; i < astring.length; ++i)
                {
                    astring[i] = new String[buf.readVarIntFromBuffer()];

                    for(int j = 0; j < astring[i].length; ++j){
                        astring[i][j] = buf.readStringFromBuffer(32767);
                    }
                }
