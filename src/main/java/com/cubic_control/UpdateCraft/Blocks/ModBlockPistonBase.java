 ...
 */
public class ModBlockPistonBase extends BlockPistonBase{	
	private boolean isSticky;
	private List<Block> pushedBlockList = new ArrayList<Block>();
	private List<int[]> pushedBlockData = new ArrayList<int[]>();
	
	public ModBlockPistonBase(boolean isSticky, String name) {
		super(isSticky);
    ...
    private void updatePistonState(World world, int x, int y, int z) {
        ...
        int side = this.getPistonOrientation(pistonMetadata);
        boolean isPowered = this.isIndirectlyPowered(world, x, y, z, side);
        if(isPowered && !isExtended(pistonMetadata)){
        	this.clearBlockLists();
            if(this.getPushableBlocks(world, x+Facing.offsetsXForSide[side], y+Facing.offsetsYForSide[side], z+Facing.offsetsZForSide[side], Facing.oppositeSide[side], side, x, y, z)<=12){
                world.addBlockEvent(x, y, z, this, 1, side);
            }
        }else if(!isPowered && this.isExtended(pistonMetadata)){
        	if(this.isSticky){
        		clearBlockLists();
        		if(this.getPushableBlocks(world, x+Facing.offsetsXForSide[side]*2, y+Facing.offsetsYForSide[side]*2, z+Facing.offsetsZForSide[side]*2, Facing.oppositeSide[side], Facing.oppositeSide[side], x+Facing.offsetsXForSide[side], y+Facing.offsetsYForSide[side], z+Facing.offsetsZForSide[side])>12){
        			return;
        		}
        	}
        	world.addBlockEvent(x, y, z, this, 0, side);
        }
    }
	@Override
	public boolean onBlockEventReceived(World world, int x, int y, int z, int extend, int side) {
        ...
        if(extend == 0){
            ...
            world.setTileEntity(x+Facing.offsetsXForSide[side], y+Facing.offsetsYForSide[side], z+Facing.offsetsZForSide[side], BlockPistonMoving.getTileEntity(MBlocks.pistonHead, side | (this.isSticky ? 8 : 0), side, true, false));
    ...
    private int getPushableBlocks(World world, int x, int y, int z, int ignoreSide, final int side, final int pistonX, final int pistonY, final int pistonZ) {    	
        ...
        while(blocksPushed < 13){   
            ...
            if(pushedBlock.equals(MBlocks.slime)){
        		for(int i = 0; i<6; ++i){
    ...
    private void pushBlocks(World world, int side, boolean extending) {
    	...
    	for(int i = 0; i<pushedBlockList.size(); ++i){
            ...
            if(extending && block.equals(MBlocks.slime)){
    ...
    private boolean canPushBlock(Block block, World world, int x, int y, int z, int pushedSide, int sideToPushTo) {
        if(block == Blocks.obsidian){
            ...
            if(block != MBlocks.piston && block != MBlocks.stickyPiston){
