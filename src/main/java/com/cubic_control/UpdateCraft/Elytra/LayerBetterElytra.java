...
import com.cubic_control.UpdateCraft.Models.ModelElytra;
...
public class LayerBetterElytra{
	private static final ResourceLocation TEXTURE_ELYTRA = new ResourceLocation(RefStrings.MODID + ":textures/entity/elytra.png");
	...
	public static void doRenderLayer(EntityLivingBase entityIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch, float scale) {
		...
		if(itemstack != null && itemstack.getItem() == MItems.elytra){
			GL11.glPushAttrib(-1);
			//int colorIndex = itemstack.hasTagCompound() && itemstack.getTagCompound().hasKey("backlytra:elytraDye", NBT.TAG_ANY_NUMERIC) ? itemstack.getTagCompound().getInteger("backlytra:elytraDye") : -1;
			
			//if(colorIndex < 0 || colorIndex == 15){
				//GL11.glColor3f(1, 1, 1);
			//}else{
				/**Cubic here, this code will break compatibility between 1.7.2 and 1.7.10**/
				//Color color = new Color(ItemDye.field_150922_c[colorIndex]);
